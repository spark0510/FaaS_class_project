#!/bin/bash
wsk action invoke bench2 -i --result
for i in $(seq 10)
do
        EXECUTE=$(date +"%s.%N") && wsk action invoke bench2 -i --result >> output_az_b2_e3.json
	...
	echo "$EXECUTE" >> execute_az_b2_e3.txt
	echo "$CREATED_TIME" >> create_az_b2_e3.txt
	echo "$START_TIME" >> start_az_b2_e3.txt

	echo "$TIME1" >> time1_az_b2_e3.txt
	echo "$TIME2" >> time2_az_b2_e3.txt
done
