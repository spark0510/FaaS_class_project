...
value_all <- read.csv("b2_e1.csv", header=F)	
value <- value_all[1]
data2_1 <- data.frame(tries, times, value)

ggplot(data2_1, aes(fill=times, y=V1, x=tries)) + geom_bar(position="stack", stat="identity")