#!/bin/bash
docker pull spark77/bench-1
for i in $(seq 10)
do
	EXECUTE=$(date +"%s.%N") && wsk action invoke bench2 -i --result >> output_az_b2_e4.json
	...
	echo "$EXECUTE" >> execute_az_b2_e4.txt
	echo "$CREATED_TIME" >> create_az_b2_e4.txt
	echo "$START_TIME" >> start_az_b2_e4.txt

	echo "$TIME1" >> time1_az_b2_e4.txt
	echo "$TIME2" >> time2_az_b2_e4.txt
